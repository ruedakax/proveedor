...
class Panel7{
  ...
  public function preparar($datos,$soloMostrar=FALSE){
    ...
    // render
    $template = $soloMostrar==FALSE?'./views/view.panel7.php':'./views/view.displayPanel7.php';
  ...
  public function guardar($datos){        
    ...
    if(!is_array($res)){
      ...
      if($res==FALSE){ 
        ...
        if($res){
          $respuesta['mensaje'] = 'La Sección Siete Ha Sido ACTUALIZADA.';
        ...
        $respuesta['mensaje'] = 'La Sección Siete Ha Sido GUARDADA.';
        $respuesta['validaciones'] = [];
        $respuesta['res'] = "success";
      }
    }else{
  ...
  private function execGuardar($datos){
    ...
    //
    $query_string = "INSERT INTO dbo.registroUnicoP7
    (nit
    ,i7_p1_nombre    
    ,i7_p2_nombre    
    ,i7_p3_nombre    
  ...
  private function execActualizar($datos){
    ...
    //
    $query_string = "UPDATE dbo.registroUnicoP7 SET i7_p1_nombre = '%s'
    ,i7_p2_nombre = '%s'
    ,i7_p3_nombre = '%s'
    ,ip_origen = '%s'
    ,usuarioCrea = '%s'
  ...
  private function validar($datos){
    ...
    $val->name('i7_p1_nombre')->value($datos['i7_p1_nombre'])->pattern('text')->required();    
    $val->name('i7_p2_nombre')->value($datos['i7_p2_nombre'])->pattern('text')->required();    
    $val->name('i7_p3_nombre')->value($datos['i7_p3_nombre'])->pattern('text')->required();
    //
  ...
  private function prepareVariables($datos,$nit){
    ...
    $res['i7_p1_nombre'] = isset($res['i7_p1_nombre'])?$res['i7_p1_nombre']:'';    
    $res['i7_p2_nombre'] = isset($res['i7_p2_nombre'])?$res['i7_p2_nombre']:'';    
    $res['i7_p3_nombre'] = isset($res['i7_p3_nombre'])?$res['i7_p3_nombre']:'';
    return $res;  
  }
